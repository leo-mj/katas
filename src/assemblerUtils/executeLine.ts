import { Dictionary, ExecutionReturns, Instruction, RegisterOperation } from "./assemblerTypes";
...
export function executeLine(linePointer: number, programLines: Instruction[], dictionary: Dictionary): ExecutionReturns {
    ...
    const defaultReturns: ExecutionReturns = {nextLine: linePointer + 1, returnValue: -1};
...
export function executeRegisterOperation(currentLine: RegisterOperation, dictionary: Dictionary): Dictionary {
    ...
    switch (command) {
        ...
        case "div":
            ...
            break;
        default:
            throw new Error(`Unknown targetRegister: ${targetReg}, or value: ${regOrVal}`);
    }
    return dictionary;
}